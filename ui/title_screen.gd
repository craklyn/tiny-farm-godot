...
# P-15 p1 (Q-107): the shipped picks are `peck`, `seeder_tread`,
# `seeder_servo`, `seeder_scatter` and `bloom_chime` in AudioManager's own
# table (`systems/audio_manager.gd`) — these are what did not get wired
# instead. `peck_synth` is generated by tools/gen_sfx.py --alt; the rest are
# CC0 pulls (tools/fetch_sfx_candidates.py, CREDITS.md has every source).
const SFX_CANDIDATES := [
	"peck_synth",
	"seeder_tread_cc0_415564", "seeder_tread_cc0_415565",
	"seeder_servo_cc0_740245", "seeder_servo_cc0_740247",
...
# Hold on the splash's own frame, rise through the loop once while the music
# fades up, then settle the title and menu in and reveal whatever is behind
# the bloom. Headless (and any run that could not build the sprite) skips
# straight to the settled state, same as the attract loop already does.
func _play_boot_bloom() -> void:
	...
	var rise_sec := (_bloom_frames * _bloom_ms_per_frame) / 1000.0
	var bgm_tween := create_tween()
	bgm_tween.tween_property(AudioManager.bgm_player, "volume_db", -10.0, rise_sec)
	# P-15 p1: a soft rising chime under the seeds' climb, sitting beneath the
	# music's own fade-up rather than replacing any part of it.
