...
| Q-109 room-size ruling | Daniel chose a whole-number room multiplier per building; the shared checkout held an unintegrated ruling while the curated card and queue disagreed. | Integrated in the queue, interior chapter, card, and ruling record. The decision test now checks the chosen rule; full HQ suite passed 45 of 45. |
