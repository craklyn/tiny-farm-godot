...
2026-09-21 initial record: design work was preserved as cards before implementation. Sections 1 and 2 have since landed. The section 3 candidate now separates review names from agent asks and displays recorded deliverable links after the question and before the recommendation/result in both work review views. Rendered fixtures cover the recorded seeder animation review, a story-night sound choice and an incomplete boot-bloom result, alongside explicit evidence-order cases. The existing decision-choice controls remain intact. This candidate still awaits landing and live visual verification; the remaining redesign sections are not claimed complete.

2026-09-21: section 1 committed as 2f28105 after explicit operator acceptance; the native review disagreement and its independent adjudication are preserved. Live checks confirm equal totals across Dashboard, navigation and Queue, and exact rendered Queue membership. Code review and fixtures establish the shared membership source for the numeric summaries. Unavailable-data and verdict-refresh transitions are fixture-tested, not live user-data mutations. Dashboard, navigation and the decision queue now consume the stable ready IDs and total from `server.waiting_on_you()`. The projection also reports why non-ready work is preparing, awaiting an owner reply, awaiting verification, scheduled or completed; an unavailable reading is not rendered as zero. Backend fixtures cover shared membership and status; operator browser checks compare live totals and ready IDs. The existing deliverable gate remains unchanged.
...
2026-09-21 section 2 implementation: `server.waiting_on_you()` now evaluates each work card's recorded preparation before adding it to the ready IDs. The projection names the missing machine-readable requirements: a short deliverable name, an inspectable evidence link, the human question, either a complete recommendation or the owner's reason that none is appropriate, and an explicit record of what the verdict does next. A completed result may request an explicit verdict without a recommendation only when its owner records that this is appropriate and why. The projection stays read-only; its preparation object makes incomplete cards visible to Work and gives the owner exact return reasons.

2026-09-24 section 7 candidate: the Dashboard leads with actions drawn from the shared attention count and current goal/project signals. The check summary separates passing, failing, unmeasured and human-verified goals; each visible issue names an owner, an available next step and its check time. The saved brief loads only when its disclosure is opened, remains open after the fetch, labels its written time and stale status, and suppresses legacy agent memory markup in the human view. Writing a new brief still requires its explicit button. A focused Chrome test exercised the click-to-fetch flow and checked that opening made one read and no generation request; the shared attention projection suite passed. The source signal cache can lag up to 60 seconds, so the Dashboard says so beside its reload control. This record describes the candidate and its local checks; live deployment and CI are separate evidence.

2026-09-25 section 9: the queue layout from 37a4fe3 (a 350 px question list, a decision pane with 16 px text and a 70-character line length, and list-then-detail with Back below 1100 px) is now matched by the pages a direct link opens. A link to one decision (`#/inbox/<id>` or `#/work/<id>`) shows only that decision rather than the whole backlog scrolled to it, and a link to one work card drops the queue introduction; both open with a Back control that returns to the page the link was followed from (questions, Work or Overview), never to another single card. Those pages use the same reading rules: 16 px text, 1.6 line spacing, prose held to 72 characters, sentence-case section labels, no accent edge on cards, and long links wrapping instead of widening the screen. Reply authors show the person's name with the shared hover card instead of a seat id. The queue also stopped widening a 375 px phone screen (its hidden task-queue tooltips were 48 px too wide), hides the options legend it had been printing, drops the question repeated under itself and the duplicated "(Recommended)" on option labels, and gives links in card text a readable blue. Browser tests render the queue and a direct decision at 1440, 1280 and a true 375 px viewport (inside a frame, because headless Chrome will not open a window narrower than 500 px), checking keyboard focus, Back, text size and line spacing, line length, muted-text contrast of at least 4.5:1, and no horizontal scrolling. Screenshots of both views against a copy of the live data were checked by eye at the same widths. Daniel has not reviewed this.

2026-09-25 section 11: walked the built redesign against a copy of the live decisions and work queue (never the live store itself, per §2's own caution against fabricated evidence) — opening the queue, reading a real decision's evidence, recording a real ruling, filing a request from "New request", commenting on a work card without a verdict, and opening a work card from a link, at 1440 and a true 375 px width. Two real gaps surfaced this way, both fixed with tests rather than filed:

- Q-122 and Q-123, the two decisions actually open on 2026-09-25, each promised "the attached boards" but showed none: their evidence images were recorded on the card's `links` field in the same shape `attachments` uses, and nothing in the queue pane or the direct decision page ever read a media type off `links` — an evidence gap §4 was supposed to close, invisible until real cards used it this way. A media-typed link is now shown as evidence wherever an attachment would be, in both views, and a plain reference link still renders on its own line; `hq/tests/test_decision_link_evidence.js` and an extended `test_decision_actions.js` cover it.
- w3b629423e60, a real "Ask first" card with no deliverable, evidence, question, recommendation or recorded consequence, read as a normal, ready-to-approve request when opened by its own direct link — full-strength Approve/Decline buttons, no warning — even though the shared readiness projection already knew exactly what was missing and says so on the list page. The one line connecting a focused card to that same reason map had never been written. It now is, and it stays silent on a card that is genuinely ready; `test_work_attention.js` covers both the warning and its absence.

Everything else checked out against the real data: Dashboard, navigation and the queue kept one shared number before and after a real ruling; ruling on Q-123 in the copy produced a correctly shaped `pending_integration` record; filing a request through "New request" produced a durable, correctly owned and tiered work card; commenting on a work card without a verdict recorded the reply and moved its status to "the studio owes a reply" live; the fixed evidence rendering holds at both 1440 and 375 px. No rival deliverable gate was added — both fixes are display-layer, reading the same backend projection w44ff2049d9a already produces. `hq/run_tests.py` passes at 57/57. Daniel has not reviewed this.
...
## Cost-conscious execution policy (Daniel, 2026-09-21)

Terra builds the remaining redesign cards, Sol reviews each once, and Luna handles routine records and writing checks. Stop on a failed card rather than automatically repeating work or full reviews. Diagnose whether the cause is the implementation, tests, brief or review finding; escalate only the affected task when greater capability is warranted. Preserve the existing work and evidence when escalating. Astra is reserved for architecture and difficult judgments. The Chief of Staff task should use Terra for routine supervision.

The runner records elapsed time, first-attempt outcome, attempt count changes and the model-usage entries added by each run. Token totals include cached tokens separately and are not a subscription-percentage or dollar-cost estimate. These metrics exclude this interactive task, its subagents and writing hooks not attributed to the card. First-pass success is only a proxy for quality; tests and review remain necessary. Model-efficiency judgment is initially an operator responsibility, not automatic routing. Repeated comparable failures should inform the default for that task type.
