...
The live service currently couples code root, repository root, and data root. Add one tested configuration path that permits code to run from the clean main checkout while its **single existing live card/ruling/patch/run store** remains at the primary checkout's `hq/data`; Git/repo facts and integration refer to the clean main worktree. Update server, drain, work, and execution-policy path resolution as one unit, or choose a simpler equally safe activation strategy. An isolated temporary-data canary must start and stop without touching live records. Only after that may the actual service file and process move to clean-main code. Record pre/post card counts, weather revision, queue pause, service health, and rollback command/config; never start a second server against live data concurrently. Do not mistake a successful offline fixture for live activation.
