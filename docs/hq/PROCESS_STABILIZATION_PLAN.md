...
- 2026-09-22 13:00 PDT — Resumed automatic starts after the live audit. `/api/execution` reported `paused: false`, an active waiting timer, 43 eligible cards, and no active worker; the Bullpen showed “Task queue Running” and “No worker is running.” Running here means the timer is enabled, not that a model is currently working.
