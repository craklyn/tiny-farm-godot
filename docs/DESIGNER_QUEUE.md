...
- ~~**Q-109** How much room a room gets~~ — **Ruled 2026-09-21: (d), choose the
  ...
  (`q109_farmhouse_multipliers.png`, `q109_coop_multipliers.png`). **The coupling is the real
  content**: the same number sets the room's size and the yard's magnification, so under
...
- **Q-101** ~~The training workbench~~ — ✅ ruled 2026-09-10: **all five pages, and a
  ...
  structure bought in the shop and set down in the yard, tapped like anything else, not as
...
- **Q-99** ~~A fresh Mark III walks off the field. Pen it, home it, or give it a nose?~~ —
  ...
  give the bot the option to till the soil. Then the bot would have more opportunities to
  do something beneficial by accident and then proceed to do something beneficial
  ongoingly."* A till action, a bare-soil channel and +0.1 for tilling; the demo's gate
  moves back to open ground, where the pen was never in the game. Consequence named in
  `design/06`: a robot can make its own practice ground.

- **Q-98** ~~What does a Mark III keep when she picks it up?~~ — ✅ ruled 2026-09-10: **the
...
- **Q-96** ~~The Mark III's first lesson: what it is rewarded for, what it can see, and
  what it costs~~ — ✅ ruled 2026-09-09: **reward +1 for a tile it made wet, 0 for
  everything else; inputs as drafted; 800 gold.** The draft's penalties for wasted water
  and refused steps were struck: *"there's no actual penalty to a human for not watering,
  so it seems strange to penalize the bot."* He framed it in Sutton and Barto's terms
...
- **Q-94** ~~How should a ripe crop announce itself?~~ — ✅ ruled 2026-09-08: **the two
  ...
  on asking for the combination: *"make the ripe ones dance slightly more subtly, but also
  have their glow."* Movement finds an eye that is looking elsewhere, light holds it once
  it arrives, and halving the sway is what stops the two competing to be the thing you
  notice.
  **Ruled past the question asked:** the switch comes out with the answer, and so do the
  four losing drafts — this project's own rule for a settled A/B ("the loser is deleted
  rather than argued about"). The sheets on the card are the record of what was compared,
  and a record does not need to be code. With it went the other three look switches, all
  of them ruled back on 2026-09-01, so the pause menu is back to two lines and the Look Lab
  registry is empty and waiting for the next question. Written up in `design/09` (the
  colour ledger, and the two rules the work produced) and `design/11` ("What altitude costs
  a cue"). Still open beside it and filed to the art director: **should each crop sway by a
...
- **Q-119** ~~Should a bottom-bar inventory button open a pictured item picker?~~
  ...
  revised card offers that direction, the shipped crop-count HUD and cycling,
  or a live touch-size comparison before a choice. Its attached text-versus-tiny-
