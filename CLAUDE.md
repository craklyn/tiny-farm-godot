...
# reaches it and prints how many of the three tomatoes survive
...
- `docs/WRITING.md` — how anything a human sees is worded (HQ pages, decision
  ...
  ask bodies, prompts, data records — is exempt and written for efficiency.
...
- `docs/marketing/streaming/` — everything known about streaming development sessions
  (OBS setup, Twitch and TikTok status, the go-live checklist, a dated log). Owned by
  Marketing: Jordan (TikTok) day to day, Carmen accountable. The OBS configuration is
  generated by `tools/streaming/obs_setup.py`; stream keys never enter the repo. Any
  streaming work starts by reading that directory and ends by adding to its `log.md`.
