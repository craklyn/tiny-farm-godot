...
const record = state => ({ wake_evaluation: {
  state,
  reason: state === 'waiting' ? 'waiting for release update-2 to become current'
    : state === 'satisfied' ? 'release update-2 has been reached'
    : "release 'missing' does not exist",
  authorized_by: { kind: 'decision', id: 'Q-90' },
} });
...
console.log('Project wait wake-up states render truthfully (5 assertions).');
